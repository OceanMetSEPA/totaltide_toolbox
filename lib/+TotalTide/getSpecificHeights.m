function tideHeights=getSpecificHeights(port,t)
    % Get tide heights for TotalTide port at specified times (rounded to nearest minute)
    %
    % This function is a wrapper for TotalTide.getHeights, which
    ...
    % calls TotalTide.getHeights.
    %
    % As such, it is useful for obtaining tide heights for irregular time-series. By
    % calling getPortHeights for each day, we can avoid getting dates for
    ...
    % h=TotalTide.getSpecificHeights(port,now) % tide height now
    % h=TotalTide.getSpecificHeights(port,ceil(now)+rand(100,1)) % 100 random times tomorrow
    %
    % For regularly spaced data, getHeights is of course a bit faster than this function,
    % but not hugely: 83s vs 90s to get a year's worth of 15 minute-spaced data
    %

    if nargin<2
        help TotalTide.getSpecificHeights
        return
    end
