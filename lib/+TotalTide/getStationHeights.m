function [ dataSeries ] = getStationHeights( station, startDate, numDays, interval)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% $Workfile:   getStationHeights.m  $
% $Revision:   1.0  $
% $Author:   Ted.Schlicke  $
% $Date:   Jul 02 2020 19:05:22  $
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    % TotalTide.getStationSlackHeights() function.
    %
    % Usage:
    % TotalTides.getStationHeights(station, startDate, numDays, interval) 
    %   where 
    %
    %     station is an instance of TotalTide::IPort. See http://www.chersoft.co.uk/totaltidesdk/reference/interface_total_tide_1_1_i_port.html
    %
    ...
    %    tide = TotalTides.getStationHeights(port,'16/05/2006',100,20)
    %
    % DEPENDENCIES:
    ...
    %  - TotalTide.dateTimeMatlabFormat.m
    %  - structFilter.m
    
    if nargin==0
      help TotalTide.getStationHeights
      return
    end
    ...
    for i = 1:numberIterations
        ...
        heights = station.Heights(days,interval);
